...
public class LaboratoryTestManager {
	...
	public void saveLaboratoryTest(LaboratoryTest labTest){
		 ...
		 // get the provider
		 Person provider = Context.getPersonService().getPerson(labTest.getProviderId());
		 ...
		 try {
			 ...
			 if(encounters.size() > 0){
				 ...
				 log.info("did not find an existing encounter so creating new one provider " + provider.getFamilyName() +  " " + provider.getPersonId());
